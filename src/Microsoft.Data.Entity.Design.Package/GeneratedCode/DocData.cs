...
namespace Microsoft.Data.Entity.Design.Package
{
    ...
    internal abstract partial class MicrosoftDataEntityDesignDocDataBase : DslShell::ModelingDocData
    {
        ...
        protected override void Load(string fileName, bool isReload)
        {
            ...
            if (modelPartition is null)
            {
                Debug.Fail("Model partition cannot be null");
            }
            var diagramPartition = this.GetDiagramPartition();
            if (diagramPartition is null)
            {
                Debug.Fail("Diagram partition cannot be null");
            }

            try
            {
                modelRoot = global::Microsoft.Data.Entity.Design.EntityDesigner.MicrosoftDataEntityDesignSerializationHelper.Instance.LoadModelAndDiagram(serializationResult, modelPartition, fileName, diagramPartition, diagramFileName, schemaResolver, this.ValidationController, this.SerializerLocator);
            }
            catch (global::System.NullReferenceException ex)
            {
                var msg = $"NullReferenceException in LoadModelAndDiagram:\n\n" +
                    $"fileName: {fileName ?? "(null)"}\n" +
                    $"diagramFileName: {diagramFileName ?? "(null)"}\n" +
                    $"modelPartition: {(modelPartition == null ? "(null)" : "OK")}\n" +
                    $"diagramPartition: {(diagramPartition == null ? "(null)" : "OK")}\n" +
                    $"schemaResolver: {(schemaResolver == null ? "(null)" : "OK")}\n" +
                    $"ValidationController: {(this.ValidationController == null ? "(null)" : "OK")}\n" +
